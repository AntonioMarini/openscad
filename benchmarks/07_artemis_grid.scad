// 5x5x5 grid of Artemis rocket models
$fn = 6;
...
module core_stage(h, r) {
    ...
    union() {
        translate([ r/2,  r/2, 0]) rs_25_engine(motors_h, r/5);
        translate([-r/2,  r/2, 0]) rs_25_engine(motors_h, r/5);
        translate([ r/2, -r/2, 0]) rs_25_engine(motors_h, r/5);
        translate([-r/2, -r/2, 0]) rs_25_engine(motors_h, r/5);
    }
...
module artemis() {
    ...
    union() {
        translate([ 3.2, 0, 0]) booster(20, 1);
        translate([-3.2, 0, 0]) booster(20, 1);
...
for (xi = [0:N-1])
  for (yi = [0:N-1])
    for (zi = [0:N-1])
      color([xi/(N-1), yi/(N-1), max(zi/(N-1), 0.15)])
        translate([(xi - (N-1)/2) * spacing_xy,
                   (yi - (N-1)/2) * spacing_xy,
                   (zi - (N-1)/2) * spacing_z])
          artemis();
