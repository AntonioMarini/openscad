...
Eigen::Vector3f OBB::half_sizes(PrimitiveType ptype) {
    ...
    case PrimitiveType::CYLINDER:
        return Eigen::Vector3f(1.0f, 0.5f, 1.0f);
    default:
        return Eigen::Vector3f::Zero();
    }
}
...
OBB OBB::buildPrimitiveOBB(const RTCSGNode& node) {
    ...
    Eigen::Vector3f he(
        scaleX * half_sizes_vec.x(),
        scaleY * half_sizes_vec.y(),
        ...
        case OperationType::INTERSECTION: {
            ...
            for (auto& tA : leftTris) {
                for (auto& tB : rightTris) {
                    ...
                    int hit = tri_tri_intersect_with_isectline(
                        tA.v0.data(), tA.v1.data(), tA.v2.data(),
                        tB.v0.data(), tB.v1.data(), tB.v2.data(),
